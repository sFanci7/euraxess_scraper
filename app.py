...
df = df[df["application_deadline"].isna() | (df["application_deadline"] >= pd.Timestamp.now())]

# Create lists of unique fields and countries
all_main_fields = []
all_sub_fields = []
for row in df.itertuples():
    if pd.notnull(row.field_1):
        all_main_fields.extend(row.field_1.split(","))
    if pd.notnull(row.field_2):
        all_sub_fields.extend(row.field_2.split(","))
...
new_columns = {_: df["field_1"].str.contains(_, case=False, na=False) for _ in all_main_fields}
df = pd.concat([df, pd.DataFrame(new_columns)], axis=1)

new_columns = {_: df["field_2"].str.contains(_, case=False, na=False) for _ in all_sub_fields}
df = pd.concat([df, pd.DataFrame(new_columns)], axis=1)
...
st.dataframe(df_filtered[show_columns], use_container_width=True, height=500, column_config={"link": st.column_config.LinkColumn()})
